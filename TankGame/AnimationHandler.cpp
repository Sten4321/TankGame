...
void AnimationHandler::update(sf::Time dt)
{
	...
	// Check if the animation has progessed to a new frame and if so change to the next frame
	if (int((t + dt.asSeconds()) / duration) > int(t / duration))
	{
		// Calculate the frame number
		int frame = int((t + dt.asSeconds()) / duration);
